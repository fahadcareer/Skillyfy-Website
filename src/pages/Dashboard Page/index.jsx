...
import { UserCheck, Users, History, Loader2 } from "lucide-react";
import useAssessmentStore from "../../store/assessment-store/assessment-store";

export default function Dashboard() {
    const navigate = useNavigate();
    const { generateAssessment, fetchPastAssessments, fetchAllAssessments, loading } = useAssessmentStore();
    ...
    const handlePastAssessments = async () => {
        ...
        if (success && data.length > 0) navigate("/past-assessments", { state: { data } });
        else alert(error || "No past assessments found.");
    };

    const handleAllUsersResults = async () => {
        const { success, data, error } = await fetchAllAssessments();
        if (success && data.length > 0) navigate("/all-users-results", { state: { data } });
        else alert(error || "No assessments found.");
    };
        ...
        <div className="min-h-screen flex flex-col bg-gray-50 relative">
            ...
            <header className="bg-white shadow-sm border-b border-gray-100">
                <div className="max-w-6xl mx-auto px-8 py-4 flex justify-between items-center">
                    <h1 className="text-2xl font-bold text-blue-600 tracking-tight">Skillyfy Dashboard</h1>
                    <button
                        onClick={() => {
                            localStorage.clear();
                            navigate("/");
            ...
            <section className="max-w-6xl mx-auto w-full px-8 py-10">
                <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-10 text-white shadow-lg">
                    <h2 className="text-3xl font-semibold mb-3">Welcome Back 👋</h2>
                    <p className="text-blue-100 text-lg">
                        Explore your assessments, track growth, and unlock personalized learning paths.
                    </p>
                </div>
            </section>
            ...
            <main className="flex-1 max-w-6xl mx-auto px-8 py-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {actions.map((item, index) => (
                        <div
                            key={index}
                            className={`bg-gradient-to-br ${item.color} rounded-2xl p-6 shadow-sm hover:shadow-lg cursor-pointer transition transform hover:-translate-y-1 ${loading ? "opacity-50 pointer-events-none" : ""
                                }`}
                            onClick={() => {
                                if (item.title === "Self Assessment") handleSelfAssessment();
                                else if (item.title === "Past Assessments") handlePastAssessments();
                                else if (item.title === "All Users Results") handleAllUsersResults();
                            }}
                        >
                            <div className="flex items-center justify-between mb-4">
                                <div className="bg-white p-3 rounded-xl shadow-inner">{item.icon}</div>
                            </div>
